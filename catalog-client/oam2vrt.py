...
class Image(object):
    ...
    @property
    def transform(self):
        return (self.left, self.px_width, 0.0, self.top, -self.px_height)
    ...
    def get_band_info(self, vrt):
        ...
        for band in tree.findall("VRTRasterBand"):
            ...
            bands[interp] = [idx, props["DataType"], props["BlockXSize"], props["BlockYSize"]]
        return bands
...
def fetch_images(bbox, opts):
    ...
    req = urllib2.Request(opts.service + "?image/?bbox=%f,%f,%f,%f" % bbox)
    try:
...
def write_vrt_source(output, target, source, band):
    ...
    if not source_win or not target_win: return
...
if __name__ == "__main__":
    ...
    # do something intelligent about masking/alpha band etc
    width, height = ...
    target = Image(output, opts.bbox, (width, height), images[0].crs) 
